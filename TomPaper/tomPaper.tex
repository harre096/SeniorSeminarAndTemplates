...
\todo[inline]{Mention EMV here or remove from "Viability ... Ticketing"}
...
Variant 2 is built upon Variant 1, but uses a reversed hash chain to reduce the frequency of fetching tokens by without substantially affecting performance. Initially, a one-way hash function is used to generate a sequence of values. Then the and only the last item is revealed. Then after every time period, the First, a one-way hash function is used to generate a sequence; revealing the items of the sequence in unveiled first first.\footnote{
...
This  that builds upon itself using the previous result as the seed for the next answer. As a result, givenThe simple token is replaced with an only slightly longer certificate, still assigned to D from CA.
 ...
 The certificate contains D's identity, a hash chain value, a hash chain index, and the same two time stamps, forming a window of validity. reduce replay risk
