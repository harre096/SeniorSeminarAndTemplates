...
Several steps occur in a standard D$\leftrightarrow$R transaction. First, R initiates communication, sending its identity and a random challenge to D. D then responds by sending back its transport ID certificate. Next, D uses its private key to generate a signature using the challenge and identities of both D and R. Finally, R verifies all of the information, checks D against its blacklist, opens the transit gate, and forwards all transaction to the TA. D also keeps a evidence of the transaction which can be used to prove ticket purchases.

\subsection{Variant 1}
In variant 1, the certificate is replace by a significantly-smaller, quickly-expiring token.
