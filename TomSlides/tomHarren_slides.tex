...
\begin{frame}
\frametitle{Proposed Secure Credit Protocol}\centering
  \includegraphics[width=\linewidth,height=.4\textheight,keepaspectratio]{../TomPaper/figures/secureCC.png}
  \newline
    \begin{minipage}[t][5mm][t]{.7\textwidth}
      \only<1>{
      ...
      \only<3>{
        \begin{block}{Restructured Card Information}
          \circled{A}~~Phones can emulate tags
          \begin{itemize}
            \item{Credit card has been r       \textbf{\textcolor{uipaleblue}{card number, expiration, bank name, and iCVV}}}
            \item{Unfortunately, this transmission is in plain text}
          \end{itemize}
        \end{block}
      }
